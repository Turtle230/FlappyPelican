...
// --- IMAGES ---
let bgImg = new Image();
bgImg.src = "birdAssets/background.png"; 

let pipeImg = new Image();
pipeImg.src = "birdAssets/pipe-green.png";

let baseImg = new Image();
baseImg.src = "birdAssets/base.png";

let gameOverImg = new Image();
gameOverImg.src = "birdAssets/gameOver.png";

let scorePanelImg = new Image();
scorePanelImg.src = "birdAssets/scoreScreen.png";

let okBtnImg = new Image();
okBtnImg.src = "birdAssets/ok_button.png";

let pelicanSheet = new Image();
pelicanSheet.src = "birdAssets/PelicanFly.png"; 
const COLS = 5;
const ROWS = 5;
...
for (let i = 0; i < 10; i++) {
    let img = new Image();
    img.src = `birdAssets/numbers/${i}.png`;
    numberImgs.push(img);
}
...
function handleAction(e) {
    if (gameOver) {
        // Calculate scaling ratio between internal resolution and screen display size
        let rect = game.getBoundingClientRect();
        let scaleX = gameWidth / rect.width;
        let scaleY = gameHeight / rect.height;

        // Map the mouse click to the internal 360x640 coordinates
...
function draw() {
    ...
    if (started && !gameOver) drawCenteredScore(score, 20);
}
...
function drawGameOver() {
    ...
    if (pelicanSheet.complete) {
        ...
        // Final Tweaks: panelX + 20 and panelY + 5
        context.drawImage(
            pelicanSheet, 
            sx, sy, sw, sh, 
            panelX + 20, panelY + 5 + bob, 
            idleW, idleH
        );
    }
